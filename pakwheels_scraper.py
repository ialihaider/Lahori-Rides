...
def get_cars():
    ...
    for domain in domains:
        try:
            ...
            # Check if we got redirected to a login/captcha page
            if 'login' in response.url.lower() or 'captcha' in response.url.lower():
                print(f"❌ Redirected to {response.url} - trying next domain")
                continue
            ...
            search_url = f"https://{domain}/used-cars/search/-/ct_lahore/pr_1600000_2500000/"
            ...
            print("💾 Saved HTML response for debugging")
            
            # Check if the response contains expected content
            if 'used cars for sale' not in response.text.lower():
            ...
            if not listings:
                print("❌ No listings found - trying next domain")
                continue
            ...
            for i, listing in enumerate(listings, 1):
                try:
                    ...
                    # Debug print the listing HTML
                    print(f"Listing HTML structure:")
                    print(listing.prettify()[:500])  # Print first 500 chars
                    ...
                    location_elem = (
                        listing.select_one('ul.search-vehicle-info li:last-child') or
                        listing.select_one('.search-vehicle-info-2 li:last-child') or
                        listing.select_one('.location-tag')
                    )
                    
                    year_elem = (
                        listing.select_one('ul.search-vehicle-info li:first-child') or
                        listing.select_one('.search-vehicle-info-2 li:first-child') or
                        listing.select_one('.year-tag')
                    )
                    
                    # Debug print what we found
                    print(f"Found elements:")
                    print(f"Title element: {title_elem is not None}")
                    print(f"Price element: {price_elem is not None}")
                    print(f"Location element: {location_elem is not None}")
                    print(f"Year element: {year_elem is not None}")
                    
                    if all([title_elem, price_elem, location_elem, year_elem]):
                        car = {
                        ...
                        cars.append(car)
                        print(f"✅ Successfully parsed: {car['title']} - {car['price']}")
                    else:
                        ...
                        if not year_elem: missing.append('year')
...
def matches_criteria(car_info):
    ...
    price = extract_price(car_info['price'])
    if price is None:
        print("❌ Could not parse price")
        return False
        
    if price > PRICE_LIMIT:
        print(f"❌ Price {price:,} PKR is above limit of {PRICE_LIMIT:,} PKR")
        return False
